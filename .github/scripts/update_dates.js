...
function getNextAvailableDate(startDate, holidays, usedDates) {
    ...
    while (date <= sixMonthsFromNow) {
        ...
        if (VALID_DAYS.includes(VALID_DAYS[date.getDay() - 1]) && 
...
function updateProbeDate(probe, newDate, content) {
    const oldDateRegex = new RegExp(`(${probe.id}[\\s\\S]*?appointment_date: ")${probe.requests[0].body.appointment_date}`, 'g');
    return content.replace(oldDateRegex, `$1${newDate}`);
}

function updateAlertMessage(probe, correctDateText, content) {
    const probeRegex = new RegExp(`(${probe.id}[\\s\\S]*?message: \\|\\n\\s+\\*).*?(\\*\\s+{{#if)`, 'g');
...
async function updateDates(filePath) {
    ...
    data.probes.forEach(probe => {
        if (probe.id.startsWith('jadwal-')) {
            ...
            if (appointmentDate > sixMonthsFromNow || usedDates.has(appointmentDate.toISOString().split('T')[0])) {
                newDate = getNextAvailableDate(currentDate.toISOString().split('T')[0], holidays, usedDates);
                content = updateProbeDate(probe, newDate, content);
                hasChanges = true;
                console.log(`Updated ${probe.id} from ${probe.requests[0].body.appointment_date} to ${newDate}`);
            ...
            if (probe.alerts) {
                ...
                hasChanges = true;
                console.log(`Updated alert message for ${probe.id} to "${correctDateText}"`);
            }
        }
    });
